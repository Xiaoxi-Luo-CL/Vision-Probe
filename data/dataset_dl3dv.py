...
class Dataset(Dataset):
    def __init__(self, config):
        ...
        self.seed = int(self.config.training.get("seed", 0))
    ...
    def preprocess_frames(self, frames_chosen, image_paths_chosen, fl_x, fl_y, cx, cy):
        ...
        for cur_frame, cur_image_path in zip(frames_chosen, image_paths_chosen):
            ...
            fxfycxcy = np.array([fl_x, fl_y, cx, cy])
    ...
    def __getitem__(self, idx):
        ...
        scene_root = scene_path.replace('transforms.json', '')
        ...
        if self.inference and scene_name in self.view_idx_list:
            ...
            deterministic_seed = int(
                hashlib.sha1(f"{scene_name}:{self.seed}".encode("utf-8")).hexdigest()[:8], 16
            )
            ...
            if image_indices is None or len(image_indices) == 0:
                if len(self) == 1:
                    raise RuntimeError(f"Scene '{scene_name}' has insufficient valid views for sampling.")
        ...
        image_paths_chosen = [it if os.path.exists(it) else it.replace("/images/", "/images_8/") for it in image_paths_chosen]
        missing_image_paths = [it for it in image_paths_chosen if not os.path.exists(it)]
        if missing_image_paths:
            if len(self) == 1:
                raise FileNotFoundError(f"Missing image paths: {missing_image_paths}")
            return self.__getitem__((idx + 1) % len(self))
        frames_chosen = [frames[ic] for ic in image_indices]
        try:
            input_images, input_intrinsics, input_c2ws = self.preprocess_frames(
                frames_chosen, image_paths_chosen, fl_x, fl_y, cx, cy
            )
